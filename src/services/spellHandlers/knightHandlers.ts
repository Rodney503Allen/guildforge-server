...
const TANK_STANCE_THREAT_GENERATION_PERCENT = 100;
...
function livingAllies(
  ...
  return allies.length > 0
    ...
    : [{
        ...
        sp:
          Number(
            context.currentPlayerSP ??
            context.player?.spoints ??
            0
          ),
...
SpellHandlerDefinition = {
  ...
  validate(spell) {
    ...
    if (
      ...
      return `${spell.name} has invalid weakening configuration`;
    }
  ...
  async execute({
    ...
    let log =
      result.dodged
        ...
        : `🛡️ ${spell.name} slams the enemy for ${damage} damage${
            result.crit
              ? " (CRITICAL!)"
              : ""
          }!`;
    ...
    if (appliedStatus) {
      log +=
        ` Its damage is reduced by ${debuff.value}% for ${debuff.duration}s.`;
    }
    ...
    return {
      ...
      /*
       * Dedicated tank attacks need to scale with
       * damage instead of relying on tiny flat values.
       */
      threatMultiplier:
...
SpellHandlerDefinition = {
  ...
  validate(spell) {
    ...
    if (
      ...
      return `${spell.name} has invalid Bulwark configuration`;
    }
  ...
  async execute(
    ...
    const threatGenerationPercent =
      Math.max(
        TANK_STANCE_THREAT_GENERATION_PERCENT,
        Math.round(
          (
            Math.max(
              1,
    ...
    return {
      log:
        `🛡️ You brace behind ${spell.name}, gaining ${buff.value} Defense, ` +
        `a ${shieldAmount}-point shield, and ${threatGenerationPercent}% increased threat generation for ${buff.duration}s!`,
...
SpellHandlerDefinition = {
  ...
  validate(spell) {
    ...
    if (
      ...
      return `${spell.name} must use intercept`;
    }
    ...
    if (
      ...
      return `${spell.name} has invalid Intercept configuration`;
    }
  ...
  async execute({
    ...
    const source =
      `knight:${playerId}:spell:${spell.id}`;
    ...
    await applyStatus(
      targetId,
      "knight_intercept_redirect_reduction",
      1,
      redirectedReduction,
      buff.duration,
      source
...
SpellHandlerDefinition = {
  ...
  validate(spell) {
    ...
    if (
      ...
      return `${spell.name} has invalid party mitigation configuration`;
    }
  ...
  async execute(
    ...
    for (
      ...
      await applyBuff(
        ...
        buff.value,
        buff.duration,
        `spell:${context.spell.id}:wall`
      );
    }

    return {
      log:
        `🛡️ ${context.spell.name} reduces incoming damage for ${targets.length} living ` +
        `${targets.length === 1 ? "ally" : "allies"} by ${buff.value}% for ${buff.duration}s!`,
...
export const unbreakableHandler:
SpellHandlerDefinition = {
  requiresEnemy: false,
  ...
  validate(spell) {
    ...
    if (
      ...
      return `${spell.name} has invalid ultimate mitigation`;
    }
    ...
    if (
      ...
      return `${spell.name} has invalid ultimate shield`;
    }
    ...
    if (
      ...
      return `${spell.name} has invalid death prevention`;
    }
  ...
  async execute(
    ...
    return {
      log:
        `🛡️ UNBREAKABLE! You gain ${buff.value}% damage reduction, a ${shieldAmount}-point shield, ` +
        `and ${charges} death-prevention ${charges === 1 ? "charge" : "charges"} for ${buff.duration}s!`,
