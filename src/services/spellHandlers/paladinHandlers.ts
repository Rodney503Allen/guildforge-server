...
const PALADIN_THREAT_GENERATION_PERCENT = 100;
...
async function applyThreatGenerationStatus(
  playerId: number,
  duration: number,
  spellId: number
) {
  ...
  await db.query(
    ...
    [
      playerId,
      PALADIN_THREAT_GENERATION_PERCENT,
      safeDuration,
      `spell:${spellId}:paladin-threat`
    ]
  );
}
...
// =====================================================
// SACRED STRIKE
// Direct holy damage plus strong tank threat.
// =====================================================

export const sacredStrikeHandler:
SpellHandlerDefinition = {
  requiresEnemy: true,
  ...
  async execute({
    ...
    return {
      log:
        result.dodged
          ? `⚔ ${spell.name} misses!`
          : `⚔ ${spell.name} strikes for ${damage} holy damage${
              result.crit
                ? " (CRITICAL!)"
                : ""
            }!`,
...
// =====================================================
// SACRED SHIELD
// Single-target max-HP absorb plus meaningful caster threat.
// Also establishes the Paladin's tank-threat state.
// =====================================================
...
SpellHandlerDefinition = {
  ...
  async execute({
    ...
    await applyThreatGenerationStatus(
      playerId,
      buff.duration,
      Number(
        spell.id
    ...
    return {
      log:
        ...
        Number(
          ...
          ? `🛡️ ${spell.name} grants you a ${shieldAmount}-point shield and empowers your threat generation for ${buff.duration}s!`
          : `🛡️ ${spell.name} grants your ally a ${shieldAmount}-point shield and empowers your threat generation for ${buff.duration}s!`,
...
export const consecrationHandler:
SpellHandlerDefinition = {
  requiresEnemy: false,
...
export const divineBulwarkHandler:
SpellHandlerDefinition = {
  requiresEnemy: false,
