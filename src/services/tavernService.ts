...
export async function getAvailableRumorQuests(pid: number, townId: number) {
  ...
  for (const row of rows) {
    ...
    const kill = (objs || []).find(
      (o: any) => o.type === "KILL" && o.target_creature_id
    );

    const turnIn = (objs || []).find(
      (o: any) => o.type === "TURN_IN" && o.target_item_id
    );
    ...
    results.push({
      ...
      objectivePreview: kill
        ? {
            type: "KILL",
            required: Number(kill.required_count),
            creature: targetCreature,
            regionHint: kill.region_name ?? null
          }
        : turnIn
        ? {
            type: "TURN_IN",
            required: Number(turnIn.required_count),
            item: targetItem,
            regionHint: turnIn.region_name ?? null
          }
        : null
    });
  }
